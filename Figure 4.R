...
for (i in 1:nrow(pars)){
  ...
  sim_data$ince <- sim_data$Ci+sim_data$Cj
  T <- length(day)
  ...
  sim_week <- sim_data%>% group_by(.id,week) %>% summarize(reports=sum(reports)) %>% as.data.frame()
  dat_list_week = split(sim_week, sim_week$.id)
  agg_week <- map_dfr(dat_list_week,~estimate_R(incid =.x$reports, 
                                           dt = 7L,
                                           dt_out = 7L,
                                           recon_opt = "naive",
                                           iter = 10L,
                                           ...
                                           config = make_config(list(
                                             mean_si = GI,
                                             std_si = sd_GI)))$R,.id=".id")
  agg_week <- agg_week[-c(1:6),]
  agg_week$.id <- as.numeric(agg_week$.id)
  day_below_aggR_report  = agg_week  %>% group_by(.id)  %>% 
    filter(`Mean(R)`<1)%>% filter(row_number()==1)
  
  thresh_error_reports_agg <- day_below_aggR_report$t_start-day_below_estimR_reports$t_start
  sim <- as.factor(rep(pars$run[i],length(thresh_error_reports_agg)))
  d_beta <- as.factor(rep(pars$dist[i],length(thresh_error_reports_agg)))
  Rt_threshold_list[[i]] <- data.frame(sim,d_beta, thresh_error_reports_agg)
  print(i)
}

Rt_threshold_df <- do.call(rbind, Rt_threshold_list)
agg.long <- melt(Rt_threshold_df, id=c("d_beta","sim"))
agg.long$d_beta <- as.factor(agg.long$d_beta)
agg.long$sim <- as.factor(agg.long$sim)
median(agg.long$value)

levels(agg.long$d_beta) <- c("1"=TeX("$\\delta_{\\beta_i} = 2 \\times 10^{-5} $"),
                                "2"=TeX("$\\delta_{\\beta_i} = 4 \\times 10^{-5} $"),
                                "3"=TeX("$\\delta_{\\beta_i} = 6 \\times 10^{-5} $"),
                                "4"=TeX("$\\delta_{\\beta_i} = 8 \\times 10^{-5} $"))

ggplot(agg.long, x=variable, aes(y=value, fill=sim)) +  
  ...
  scale_fill_brewer(palette="Reds",labels=c('Single Pop.', TeX("$1 \\times 10^{-5}$"),
