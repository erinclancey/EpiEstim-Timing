...
##EpiEstim R_t
sim_data$reports <- sim_data$reports_i
T <- length(day)
...
subpop <- ggplot(thresh.long, x=variable, aes(y=value, fill=sim)) +  
  ...
  scale_fill_brewer(palette="Blues",labels=c('Single', TeX("$1 \\times 10^{-5}$ "),
                                             TeX("$1 \\times 10^{-6}$ "),TeX("$1 \\times 10^{-7}$ "),
                                             TeX("$1 \\times 10^{-8}$ "),"Separate"))+
theme(axis.text.x=element_blank(),
  ...
  ylab(TeX("\\delta$_t$ when \\textit{\\hat{R}}$_{i_t}$<1"))+
  xlab(TeX("Population Structure $\\rightarrow$"))+
